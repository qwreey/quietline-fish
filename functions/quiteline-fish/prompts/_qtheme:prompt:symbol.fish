function _qtheme:prompt:symbol \
--argument-names symbol
    set --query _qtheme_var_symbol_id
    or set --global _qtheme_var_symbol_id "0"
    set --global _qtheme_var_symbol_id (math $_qtheme_var_symbol_id + 1)
    ...
    echo "namespace  =$symbol"
    # namespace 를 담는 array 도 만들어서
    # render 나 hook 호출 마다 _qtm_ns 가 업데이트 되도록
